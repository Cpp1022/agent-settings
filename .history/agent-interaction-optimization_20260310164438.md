...
- specs/规则.md 写什么 ✅ **已决策，详见 `specs/规则.md` v1**
> ~~支持规则更新后agent及时应用~~
    - ✅ 方案：规则.md 头部版本号 + memory 版本比对；规则文件保持精简(≤500行)每次全量读取，约1-2k token可接受
> ~~任务完成后记录完成过程，作为后续参考~~
    - ✅ 方案：完成后自动提炼 skill（memory MCP + specs/skills/）；决策过程写入 process/task-log
> ~~修改完成方式等各种时及时修改对应文档~~
    - ✅ 方案：变更联动规则表，见 `specs/规则.md` §3
> ~~文件检查与写入规则~~
    - ✅ 方案：内容去重、归属规则、引用决策规则，见 `specs/规则.md` §4
